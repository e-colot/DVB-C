...
A similar test was done for the CFO estimator and the results are shown in figure \ref{fig:CFO_std_CFO}. The CFO estimator is as robust as the time of arrival estimator as they are built together. Its estimation starts to get further away from the real CFO only for a big carrier frequency offset value. \\

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{cfo_vs_cfo.eps}
    \caption{CFO estimation with varying CFO}
